/* eslint-disable react/no-unescaped-entinties */
'use client';
...
const langs = {
    order: {
        tr: 'SIRA', ua: 'ПОРЯДОК', en: 'ORDER',
    }, image: {
        tr: 'RESİM', ua: 'ЗОБРАЖЕННЯ', en: 'IMAGE',
    }, productFeatures: {
        tr: 'ÜRÜN ÖZELLİKLERİ', ua: 'ХАРАКТЕРИСТИКИ ТОВАРУ', en: 'PRODUCT FEATURES',
    }, price: {
        tr: 'FİYAT', ua: 'ЦІНА', en: 'PRICE',
    }, quantity: {
        tr: 'ADET', ua: 'КІЛЬ', en: 'QUANTITY',
    }, total: {
        tr: 'TOPLAM', ua: 'СУМА', en: 'TOTAL',
    }, notFound: {
        tr: 'Bulunamadı', ua: 'Не знайдено', en: 'Not Found',
    }, teklifNo: {
        tr: 'Teklif no', ua: 'Тендер №', en: 'Quotation No',
    }, invoice: {
        tr: 'Teklif', ua: 'Рахунок', en: 'Invoice',
    }, date: {
        tr: 'Tarih', ua: 'Дата', en: 'Date',
    }, firmaBilgileri: {
        tr: 'Firma Bilgileri', ua: 'Інформація про компанію', en: 'Company Information',
    }, toplam: {
        tr: 'Toplam', ua: 'Всього', en: 'Total',
    }, vergi: {
        tr: 'Vergi', ua: 'Податок', en: 'Tax',
    }, genelToplam: {
        tr: 'Genel Toplam', ua: 'Загальна сума', en: 'Grand Total',
    }, indirmeButonu: {
        tr: 'Teklifi İndir', ua: 'Завантажити пропозицію', en: 'Download Offer',
    },
};
...
const Invoice = ({data, lang}) => {
    ...
    function filterDataByOrderId() {
        ...
        const res = {
            order_no: data.orderCode, musteri: {
        ...
        data.Orders.forEach((value) => {
            ...
            data.Ürünler
                ...
                .forEach((x_m) => {
                    ...
                    x_d = {
                        ...
                        name_en: x_m.productNameEN,
                        name_tr: x_m.productNameTR,
                        name_ua: x_m.productNameUA,
                        info_en: x_m.productTypeEN,
                        info_tr: x_m.productNameTR,
                        info_ua: x_m.productTypeUA
                    };
                });
            ...
            data.Extralar?.filter((x_f) => x_f.orderId === pr_id)?.forEach((x_k) => {
                x_d = {
                    ...x_d, extras: x_d?.extras ? [...x_d.extras, {
                        name: 'Extra',
                        value: x_k.extravalue,
                        tr: x_k.productNameTR,
                        en: x_k.productNameEN,
                        ua: x_k.productNameUA
                    },] : [{
                        name: 'Extra',
                        value: x_k.extravalue,
                        tr: x_k.productNameTR,
            ...
            data.Kumaşlar?.filter((x_f) => x_f.orderId === or_id)?.forEach((x_k) => {
                ...
                x_d = {
                    ...x_d, extras: x_d?.extras ? [...x_d.extras, {
                        ...
                        tr: x_k.fabricSwatchTurkish,
                        en: x_k.fabricSwatchEnglish,
                        ua: x_k.fabricSwatchUkrainian
                    },] : [{
                        name: 'Kumaş',
                        value: x_k.fabricType,
                        tr: x_k.fabricSwatchTurkish,
            ...
            data.Metaller?.filter((x_f) => x_f.orderId === or_id)?.forEach((x_m) => {
                ...
                x_d = {
                    ...x_d, extras: x_d?.extras ? [...x_d.extras, {
                        name: 'Metal',
                        value: x_m.metalType,
                        tr: x_m.metalTypeTurkish,
                        en: x_m.metalTypeEnglish,
                        ua: x_m.metalTypeUkrainian
                    },] : [{
                        name: 'Metal',
                        value: x_m.metalType,
                        tr: x_k.metalTypeTurkish,
                        en: x_k.metalTypeEnglish,
                        ua: x_k.metalTypeUkrainian
                    },],
                };
            });
            ...
            data.Renkler.filter((x_f) => x_f.orderId === or_id).forEach((x_m) => {
                ...
                x_d = {
                    ...x_d, extras: x_d?.extras ? [...x_d.extras, {
                        name: 'Renk', value: x_m.colourHex, tr: null, en: null, ua: null
                    },] : [{
                        name: 'Renk', value: x_m.colourHex, tr: null, en: null, ua: null
                    },],
                };
            ...
            data.Ölçüler
                ...
                .forEach((x_m) => {
                    ...
                    x_d = {
                        ...x_d, extras: x_d?.extras ? [...x_d.extras, {
                            name: 'Ölçü', value: formated, tr: null, en: null, ua: null
                        },] : [{
                            name: 'Ölçü', value: formated, tr: null, en: null, ua: null
                        },],
                    };
    ...
    const handlePrint = useReactToPrint({
        content: () => printRef.current, pageStyle: `margin: 2.5%`
    });
    ...
    const formatCurrency = (amount) => {
        return new Intl.NumberFormat('ua-UA', {
            currency: 'UAH', minimumFractionDigits: 2,
        }).format(amount);
    };
    ...
    const calculateTotals = (taxRate, products) => {
        ...
        return {
            total: formatCurrency(total), tax: formatCurrency(tax), grandTotal: formatCurrency(grandTotal),
        };
    };
    ...
    return (<div className='flex flex-col h-fit overflow-auto gap-6 relative m-auto w-[29.7cm]'>
        ...
        <div ref={printRef} className='a4 overflow-y-auto'>
            ...
            <table className='w-full break-inside-auto'>
                ...
                <thead className='[&_tr_th]:text-center  [&_tr_th]:text-black [&_tr_th]:font-bold'>
                <tr className='h-8'>
                    <th className='px-1 w-fit !font-serif'>{langs.order[lang]}</th>
                    <th className='px-1 w-min !font-serif header-image'>{langs.image[lang]}</th>
                ...
                {details?.products?.map((product, idx) => (<tr
                    key={idx}
                    className='even:bg-[#F2F2F2] border-2 border-black bg-white break-inside-avoid'
                >
                    <td className='text-[13.5pt] text-[#363B46] font-bold border-r-2 border-black'>
                        {idx + 1}
                    </td>
                    <td className="border-r-2 border-black product-image">
                        {product?.image ? (<Image
                            src={product?.image}
                            height={75}
                            alt=''
                    ...
                    <td className='overflow-hidden border-r-2 border-black'>
                        <div className='!max-h-fit overflow-hidden gap-2 flex flex-wrap'>
                    <span className='px-2.5 py-1 bg-white text-[9pt] text-black font-bold rounded-full w-fit'>
                                    {product["name_" + lang] || product.name}
                    </span>

                            <span
                                className='px-2.5 py-1 bg-white text-[9pt] text-black font-bold rounded-full w-fit'>
                      {product["info_" + lang] || product.info}
                    </span>

                            {product?.extras?.map((feature, idx) => (<div
                                key={'feature-' + idx}
                                className='px-1 text-center align-middle flex gap-1 [&_span]:text-black [&_span]:font-bold rounded-full [&_span]:text-[10pt] items-center'
                            >
                                <span>{feature[lang] || feature.value}</span>
                            </div>))}

                            {product.note && product.note !== '' && (<div
                                className='px-2 py-1 flex gap-1 [&_span]:text-black [&_span]:font-bold rounded-md [&_span]:text-[10pt] items-center w-full'>
                                <span>{product.note}</span>
                            </div>)}
                        </div>
                    </td>
                    <td className='border-r-2 border-black whitespace-nowrap  font-medium'>
            ...
            <span className='text-[#363B46] text-[15pt] font-bold'>
              ...
              <span className='text-[#363B46] text-[13pt] font-bold'>
                {langs.vergi[lang]} :
                    ...
                    <div className='flex items-center gap-6 w-[300px] p-1.5 rounded-sm'>
              <span className='text-[#363B46] text-[13pt] font-bold'>
